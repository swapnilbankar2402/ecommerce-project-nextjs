...
// Update the form schema to include email and password
const vendorFormSchema = z.object({
    email: z.string().email('Please enter a valid email'),
...
// Update steps to include account creation
const steps = [
    {
        id: 'account',
...
export default function VendorApplicationForm() {
    ...
    const { watch, trigger, formState: { isValid, dirtyFields }, setValue, getValues } = form;
    const watchedValues = watch();
    const payoutMethod = watch('payoutInfo.method');
    const emailValue = watch('email');

    // Check if vendor exists when email changes
    useEffect(() => {
        const checkVendorExists = async () => {
            if (emailValue && emailValue.includes('@')) {
                try {
                    const response = await api.get(`/api/vendors/check-email?email=${encodeURIComponent(emailValue)}`);
                    if (response.data.success && response.data.exists) {
                        setExistingVendor(response.data.vendor);
                        toast.info('An account with this email already exists. Please log in instead.');
                    } else {
        ...
        const debounceTimer = setTimeout(checkVendorExists, 500);
    ...
    const nextStep = async () => {
        ...
        // Special handling for account step
        if (currentStep === 0) {
    ...
    const getFieldsForStep = (step: number): FieldPath<VendorFormValues>[] => {
        switch (step) {
            ...
            case 1: return ["businessName", "businessType", "taxId"];
            case 2: return ["contact.name", "contact.phone", "contact.email"];
            case 3: return ["storeName", "storeDescription"];
            case 4: return ["logo", "banner"];
            case 5: return ["policies.shipping", "policies.returns", "policies.warranty"];
            case 6: {
                const baseFields = [PAYOUT_FIELDS.method];
                if (payoutMethod === 'bank') {
                    return [
    ...
    const onSubmit = async (data: VendorFormValues) => {
        console.log("vendor application form data :", data)
        setIsSubmitting(true);
        try {
            const vendorData = {
                email: data.email,
    ...
    const handleSubmitApplication = async () => {
        ...
        // Then validate only the relevant payout fields based on the selected method
        const payoutFields = getFieldsForStep(6); // Step 6 is payout step
        const arePayoutFieldsValid = await trigger(payoutFields);
                    ...
                    <Form {...form}>
                        <form onSubmit={(e) => e.preventDefault()} className="space-y-8">
                            ...
                            <div className="min-h-[400px]">
                                {currentStep === 0 && (
                                    <div className="space-y-6">
                                        <div>
                                            ...
                                            <p className="text-muted-foreground mb-6">
                                                Create your vendor account with email and password
                                            </p>
                                        </div>
                                            ...
                                            <Card className="border-yellow-200 bg-yellow-50 mb-6">
                                                <CardContent className="pt-6">
                                                    <div className="flex items-start space-x-4">
                                                        ...
                                                        <div>
                                                            <h3 className="font-medium text-yellow-800">Account Already Exists</h3>
                                            ...
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem>
                                                        ...
                                                        <FormDescription>
                                                            This will be your vendor account email
                                                        </FormDescription>
                                                        <FormMessage />
                                                    </FormItem>
                                                )}
                                ...
                                {/* Keep the rest of the steps the same, just increment the step numbers by 1 */}
                                {currentStep === 2 && (
                                    <div className="space-y-6">
                                        <div>
                                            <h2 className="text-xl font-semibold mb-4">Contact Information</h2>
